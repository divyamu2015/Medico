...
class BookAppointmentScreen extends StatefulWidget {
  const BookAppointmentScreen({super.key, required this.clinicDoctorId});
...
String convertTo24HourNoSuffix(String time) {
  ...
  Future<void> confirmSlot() async {
    ...
    context.read<ClidocCreateSlotBloc>().add(
  ClidocCreateSlotEvent.uploadClinicSlots(
  ...
  Future<void> postAvailability(bool available) async {
    final url = Uri.parse(
      "https://417sptdw-8002.inc1.devtunnels.ms/userapp/clinic-doctor/$clinicDoctorId/availability/",
    );
  ...
  Future<void> getDoctorDetails() async {
    final url = Uri.parse(
      'https://417sptdw-8002.inc1.devtunnels.ms/userapp/view_clinic_doctor/$clinicDoctorId/',
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<ClidocCreateSlotBloc, ClidocCreateSlotState>(
                              ...
                              ? CircleAvatar(
                                  backgroundImage: NetworkImage(
                                    doctorDetails!['image']
                                            ...
                                            .startsWith('http')
                                        ? doctorDetails!['image']
                                        : 'https://417sptdw-8002.inc1.devtunnels.ms${doctorDetails!['image']}',
                                  ),
                                  radius: 20,
                                )
                              : CircleAvatar(
